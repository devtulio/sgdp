# SGDP v1.47.13 — Servidor local: SQLite, autenticação, REST API, uploads de PDF
import http.server
import socket
import sys
import os
...
def init_db():
    with get_db() as conn:
        ...
        ''')
        # Assinatura digital ICP-Brasil removida (v: ver CHANGELOG) — descarta a tabela
        # e seus dados; a assinatura manuscrita nos documentos impressos permanece.
...
class SGDPHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def _route_post(self, p, s):
        if p == '/api/auth/logout':
            ...
            # Importação em lote é restrita ao administrador: cria vários documentos
            # de uma vez e consome a numeração automática de cada tipo. Cadastrar um
            # documento continua liberado a todos.
            if not s['admin']:
        ...
        with get_db() as conn:
            rows = conn.execute(
                '''SELECT usuario_nome, acao, COUNT(*) n FROM auditoria
                   WHERE em BETWEEN ? AND ? AND acao IN ('criar','editar','upload')
                   AND usuario_nome IS NOT NULL GROUP BY usuario_nome, acao''',
                (de, ate)).fetchall()
        por_usuario = {}
        for r in rows:
            u = por_usuario.setdefault(r['usuario_nome'], {'criar': 0, 'editar': 0, 'upload': 0})
            u[r['acao']] = r['n']
    ...
    def _import_csv(self, body, s):
        # A restrição a administrador fica na linha da rota, como nas demais rotas
        # restritas da família — regra escondida no meio do handler passa
        # despercebida em auditoria.
        data = json.loads(body) if body else {}
